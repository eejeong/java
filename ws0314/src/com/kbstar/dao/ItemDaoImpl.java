...
public class ItemDaoImpl implements DAO<String, String, Item> {
	...
	public void insert(Item v) throws Exception {
		try (Connection con = getConnection(); PreparedStatement pstmt = con.prepareStatement(Sql.itemInsertSq1);) {
	...
	public void delete(String k) throws Exception {
		try (Connection con = getConnection(); PreparedStatement pstmt = con.prepareStatement(Sql.itemDeleteSq1);) {
			pstmt.setString(1, k);
	...
	public Item select(String k) throws Exception {
		Item item = null;
		try (Connection con = getConnection(); PreparedStatement pstmt = con.prepareStatement(Sql.itemSelectSq1);) {
			pstmt.setString(1, k);
...
//			e.printStackTrace(); //네트워크 문제일때
	...
	@Override
	public List<Item> selectListAll() throws Exception {
		List<Item> itemList = new ArrayList<>();
		try (Connection con = getConnection(); PreparedStatement pstmt = con.prepareStatement(Sql.itemSelectAllSq1)) {
			try (ResultSet rset = pstmt.executeQuery()) {
				while (rset.next()) {
